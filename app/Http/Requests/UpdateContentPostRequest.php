...
class UpdateContentPostRequest extends FormRequest
{
    ...
    {
        return Auth::check() && $this->user()->can('update', $this->route('content_post'));
    }
    ...
    public function rules(): array
    {
        ...
        return [
            ...
            // Image validation (optional for updates)
            'image' => [
                'nullable',
                'file',
                'image',
            ...
            // Media files validation (optional for updates)
            'media' => [
                'nullable',
                'array',
                "max:{$maxFiles}"
            ...
            // Media file removal (array of IDs to remove)
            'remove_media' => 'nullable|array',
            'remove_media.*' => 'integer|exists:content_post_media,id',
        ];
    }
    ...
    public function messages(): array
    {
        ...
        return [
            ...
            // Media removal validation messages
            'remove_media.*.integer' => 'Invalid media file ID.',
            'remove_media.*.exists' => 'Selected media file does not exist.',
        ];
    }
    ...
    public function attributes(): array
    {
        return [
            ...
            'remove_media.*' => 'media file to remove',
        ];
    }
    ...
    public function withValidator($validator): void
    {
        $validator->after(function ($validator) {
            ...
            // Validate that media files to remove belong to this content post
            if ($this->has('remove_media')) {
                $contentPost = $this->route('content_post');
                foreach ($this->input('remove_media', []) as $mediaId) {
                    if (!$contentPost->media()->where('id', $mediaId)->exists()) {
                        $validator->errors()->add('remove_media', 'One or more media files do not belong to this content post.');
                        break;
                    }
                }
            }
